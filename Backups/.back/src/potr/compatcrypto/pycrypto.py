...
class ECDSAKey(common.PK):
	...
	# Si key est spécifié la clé est crée depuis le paramètre, sinon elle est auto-générée
    def __init__(self, key=None):
        ...
        # Récupération
        if key is not None:
        	SigningKey.from_string(key, curve=SigningKey.NIST192p)
        
        # Auto-génération	
        else:
        	self.secretKey = SigningKey.generate(curve=SigningKey.NIST192p)
